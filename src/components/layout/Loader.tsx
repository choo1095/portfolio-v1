    ...
    const _animate = async () => {
      await animate([
        [
          "#loader_name div",
          { transform: "translateY(0)", opacity: 1 },
          { delay: stagger(0.04), ease: "easeInOut" },
        ],
        [
          "#loader_message div",
          { transform: "translateY(0)", opacity: 1 },
...
const LoaderPercentage = (props: Props) => {
  ...
  return (
    <div className={clsx("text-lg sm:text-xl", props.className)}>
      <div className="flex gap-2 items-center">
        <div className="mt-0.5 bg-stone-600 w-2 h-2 animate-pulse rounded-full"></div>
        <span>{number.toFixed(0)}%</span>
      </div>
    </div>
  );
};
    ...
    <div className="bg-stone-300 relative w-dvw h-dvh" ref={scope}>
      ...
      <div className="screen_container h-fit w-dvw relative top-1/2 -translate-y-full left-0 pb-3">
        <div className="sm:flex sm:items-end sm:justify-between">
          <div>
            <h1 id="loader_name" className="flex gap-1 text-3xl sm:text-4xl">
              <motion.div initial={initial}>Choo </motion.div>
              <motion.div initial={initial}>En-Ni</motion.div>
            </h1>
            <div
              id="loader_message"
              className="flex gap-1 text-sm sm:text-base"
            >
              <motion.div initial={initial}>I </motion.div>
              <motion.div initial={initial}>make </motion.div>
              <motion.div initial={initial}>websites </motion.div>
              <motion.div initial={initial}>and </motion.div>
              <motion.div initial={initial}>mobile </motion.div>
              <motion.div initial={initial}>apps</motion.div>
              <motion.div initial={initial}>.</motion.div>
            </div>
          </div>
          <LoaderPercentage className="hidden sm:block"></LoaderPercentage>
        </div>
      </div>
